...
def mod_runs(mc_df, mod_type):
    ...
    mod_time_pst = pd.read_csv('../NWM-water-temperature/model_calibration/mod_time_pst_cal.csv')
    ...
    for l in range(0,2):
        ...
        ## Feed loop different monte carlo dataframe based on configuration
            ...
            # Extract monte carlo sample
            mc_sample = mc_df_M3.iloc[l,0:9]
            
        elif mod_type == "M4":
            
            # Extract monte carlo sample
            mc_sample = mc_df.iloc[l,0:12]
    
        # Run first model computation cycle: Fa, Fb, Fc, Fd reaches
        T_Fa, Q_Fa, mod_time_Fa, nodes_Fa = nwm_st('Fa',node_space, time_step, mc_sample,'cal')
        T_Fb, Q_Fb, mod_time_Fb, nodes_Fb = nwm_st('Fb',node_space, time_step, mc_sample,'cal')
        T_Fc, Q_Fc, mod_time_Fc, nodes_Fc = nwm_st('Fc',node_space, time_step, mc_sample,'cal')
        T_Fd, Q_Fd, mod_time_Fd, nodes_Fd = nwm_st('Fd',node_space, time_step, mc_sample,'cal')
        ...
        T_Sa, Q_Sa, mod_time_Sa, nodes_Sa = nwm_st('Sa', node_space, time_step, mc_sample, 'cal',Fc=Fc_mat)
        
        # Combine T and Q at final node of tributaries into single matrix
        Sa_mat = np.vstack([T_Sa[:,-1], Q_Sa[:,-1]]).T
        
        # Run third model computation cycle: Ma reach
        T_Ma, Q_Ma, mod_time_Ma, nodes_Ma = nwm_st('Ma', node_space, time_step, mc_sample, 'cal',Fa=Fa_mat, Fb=Fb_mat,Fd=Fd_mat,Sa=Sa_mat)
...
M1.to_csv('../NWM-water-temperature/model_calibration/M1_calibration.csv', index=False)        
M1_st_Fa.to_csv('../NWM-water-temperature/model_calibration/st_runs/M1_st_headwater_cal.csv', index=False)        
M1_st_Ma.to_csv('../NWM-water-temperature/model_calibration/st_runs/M1_st_outlet_cal.csv', index=False) 

M2.to_csv('../NWM-water-temperature/model_calibration/M2_calibration.csv', index=False)        
M2_st_Fa.to_csv('../NWM-water-temperature/model_calibration/st_runs/M2_st_headwater_cal.csv', index=False)        
M2_st_Ma.to_csv('../NWM-water-temperature/model_calibration/st_runs/M2_st_outlet_cal.csv', index=False) 

M3.to_csv('../NWM-water-temperature/model_calibration/M3_calibration.csv', index=False)        
M3_st_Fa.to_csv('../NWM-water-temperature/model_calibration/st_runs/M3_st_headwater_cal.csv', index=False)        
M3_st_Ma.to_csv('../NWM-water-temperature/model_calibration/st_runs/M3_st_outlet_cal.csv', index=False)

M4.to_csv('../NWM-water-temperature/model_calibration/M4_calibration.csv', index=False)        
M4_st_Fa.to_csv('../NWM-water-temperature/model_calibration/st_runs/M4_st_headwater_cal.csv', index=False)        
M4_st_Ma.to_csv('../NWM-water-temperature/model_calibration/st_runs/M4_st_outlet_cal.csv', index=False) 
